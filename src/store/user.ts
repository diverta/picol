...
class User extends VuexModule implements IUserState {
  ...
  get getImage(): (memberId: number) => string {
    return (memberId) => {
      const user = this.users.find(({ member_id }) => member_id === memberId) || ({} as any);
      return !!user && user.member_photo.url !== false
        ? (user.member_photo.url as string)
        : CONSTANTS.UNKNOWN_USER_IMAGE_PATH;
    };
  }

  get myImage(): string {
    return !!this.selfUser && this.selfUser.member_photo.url !== undefined
      ? (this.selfUser.member_photo.url as string) + '&v=' + Math.random() // remove cache
      : CONSTANTS.UNKNOWN_USER_IMAGE_PATH;
  }
